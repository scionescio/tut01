  ...
  if (typeof title != "undefined") {
    ...
    for (var i = 0; i < images.length; i++) {
      for (var j = 0; j < images[i].attributes.length; j++) {
        if (
          images[i].attributes[j].name == "alt" &&
          images[i].attributes[j].value == title.innerHTML
        ) {
          points = points + 50;
        }
      }
...
readdir(path)
  ...
  .then(files => {
    return files.map(file => [
      fs.readFileSync(path + "/" + file + "/index.html", "utf8"),
      file
    ]);
  })
  .then(contents => {
    return contents.map(content => [
      content[1],
      evaluate.checkHTML(content[0]) + checkImage(content[0]) + seo(content[1])
    ]);
  })
  .then(results => {
    results.forEach(result =>
      console.log(`Gruppe: ${result[0]} hat Punkte: ${result[1]}`)
    );
  })
